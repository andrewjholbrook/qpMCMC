...
\def\paperTitle{Generating MCMC proposals by randomly rotating the regular simplex}
\def\paperID{JMVA-D-21-00468}
...
We appreciate the thoughtful comments and recommendations made by the Reviewers and the opportunity to revise our manuscript ``\paperTitle" (\paperID).
We have carefully considered these extremely helpful suggestions and have addressed them to the best of our abilities.  We believe that the manuscript is now \emph{significantly} improved and are thankful for the time and energy put forth by the Reviewers.

In particular, we have:
\begin{itemize}
	\item expanded the Introduction to better discuss related work, motivate our work and provide background on items such as the simplex and Haar measure;
	\item  further elucidated the relationship between the simplicial sampler and the two proposal mechanisms of \citet{tjelmeland2004using}, adding an additional result proving that strategy `P1' also enables simplified acceptance probabilities and demonstrating significant theoretical and practical speedups;
	\item added a new section, `Computational considerations' outlining aspects that will influence algorithmic effectiveness, e.g., theoretical complexity, memory constraints, parallelization, programming language and specific form of target distribution.
	\item specified why the conclusions of the now Lemma 2 do not extend to the extra-dimensional sampler when it is interpreted as using a degenerate preconditioner; and
	\item included a new numerical comparison between the simplicial sampler and the (fast and slow) P1 sampler and revisited a multiple-try Metropolis simulation using new target values.
\end{itemize}


The document below contains our replies (in \textbf{bold}) alongside the AE and Reviewers' comments (normal text).  We list changes to the text with indented normal text set off by quotation marks.
