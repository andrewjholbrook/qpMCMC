...
&= \frac{1}{\sqrt{N}}\sum_{x} \left( (-1)^{1-\delta_{x_0}(x)} + 2 \frac{(N-1)}{N} \right)\ket{x}\\ &= \frac{(3N-2)}{N^{3/2}} \ket{x_0} + \sum_{x\neq x_0} \frac{(N-2)}{N^{3/2}}  \ket{x} \, .
\end{align*}
The scientist who measures the state at this moment would obtain the desired state $\ket{x_0}$ with slightly higher probability than the other individual states.
