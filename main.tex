...
\title{A quantum parallel parallel Markov chain Monte Carlo}
\date{}
...
\begin{algorithm}[t!]
	...
	\For{$s \in\{1,\dots,S\}$}{
		$\sigma_\pi \gets 0$\;
		\For{$p' \in \{0,\dots,P\}$}{
			\uIf{$p'=0$}{$\ttheta_{p'} \gets \ttheta^{(s-1)} $}
			\Else{	$\ttheta_{p'} \gets Q(\ttheta^{(s-1)},\cdot)$\;}
			\For{$p'' \in \{0,\dots,p'-1,p'+1,\dots,P\}$}{
				$q_{p'p''} \gets q(\ttheta_{p'},\ttheta_{p''})$\;
			}
			$\pi_{p'} \gets \pi(\ttheta_{p'}) \prod_{p''\neq p'} q_{p'p''}$\;
			$\sigma_\pi \gets \sigma_\pi + \pi_{p'}$\;
		}
		$\ppi \gets \ppi / \sigma_\pi$\;
		$p \gets Discrete(\ppi)$\;
...
\begin{algorithm}[t!]
	\caption{Grover's algorithm for quantum search}\label{alg:grover}
	\KwData{An oracle gate $\U_f$ taking $\ket{x}\ket{y}$ to $\ket{x}\ket{y\oplus f(x)}$ for a function $f(x):\{0,\dots,N-1\}\rightarrow \{0,1\}$ that satisfies $f(x_0)=1$ for a single $x_0$; $n+1=\log_2(N)+1$ quantum states initialized to $\ket{0}^{\otimes n} \ket{1}$; an integer $R>\pi\sqrt{N}/4$ denoting the number of Grover iterations.}
	\KwResult{An $n$-bit binary string $x_0$ satisfying $f(x_0)=1$ with error less than $1/N$.}
	$\H^{\otimes n+1}\ket{0}^{\otimes n}\ket{1} \longrightarrow \ket{h}\ket{-}$; \hspace{11.5em} \Comment{\hspace{0.3em}$\ket{h}=\frac{1}{\sqrt{N}} \sum_{x=0}^{N-1} \ket{x}$.} 
...
\end{align*}
The scientist who measures the state at this moment would obtain the desired state $\ket{x_0}$ with a slightly higher probability of $(3N-4)^2/N^3$  than the individual probabilities of $(N-4)^2/N^3$ for the other states. It turns out that each additional application of the \emph{Grover iteration}
\begin{align*}
