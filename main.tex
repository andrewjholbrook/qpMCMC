...
\subsection{Symmetric proposals}
...
\begin{algorithm}[t!]
	...
	\For{$s \in\{1,\dots,S\}$}{
		$\ttheta_0 \gets \ttheta^{(s-1)}$\;
		$\bar{\ttheta} \gets Normal_D(\ttheta_0,\SSigma)$; \hspace{15em} \Comment{ \hspace{0.3em}     Proposal \eqref{alg:min}}      
		\For{$p \in \{0,\dots,P\}$}{
			$\ttheta_p \gets Normal_D(\bar{\ttheta},\SSigma)$\;
			$z_{p} \gets Gumbel(0,1)$\;
		}
		$\hat{p} \gets \argmin_{p=0,\dots,P}\Big(f(p):= -\big(z_{p}+ \log\pi(\ttheta_{p}) \big)\Big)$; \hspace{3.5em} \Comment{$\order{\sqrt{P}}$ Alg \ref{alg:min}}
...
Algorithm \ref{alg:qpMCMC} presents the details of QPMCMC.  The algorithm uses conventional (perhaps parallel) computing almost entirely, excluding the one line that calls the quantum minimization algorithm presented in Algorithm \ref{alg:min}.  To keep the conventional computing burden low, we recommend 
