...
Nonetheless, there are major technical barriers to the practical implementation of QPMCMC.  The framework, like other hybrid quantum machine learning schemes, requires on-loading and off-loading classical data to and from a quantum machine.  In the seminal review on the subject quantum machine learning, \citet{biamonte2017quantum} discuss the `input problem' in the unnumbered section entitled \textbf{Reading classical data into quantum machines}.  QPMCMC is an overarching framework that is valid for multiple data representations, and different representations suffer from the input problem more than others.     On the one hand, continuous targets require reading $\order{P}$ real vectors onto the quantum machine.  If one follows \citet{jordan2005fast} and opts for a fixed precision representation of each $\ttheta_p\in \mathbb{R}^D$

Conveniently, the `output problem' is less of an issue for QPMCMC, as only a single integer $\hat{p} \in \{0,\dots,P\}$ need be extracted.
