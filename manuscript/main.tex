...
\def\HiLi{\leavevmode\rlap{\hbox to \hsize{\color{trevorblue!15}\leaders\hrule height 0.9\baselineskip depth .5ex\hfill}}}
...
\begin{algorithm}[H]
	\caption{Parallel (multiproposal) MCMC \citep{tjelmeland2004using}}\label{alg:pMCMC}
...
\begin{figure}[!t]
	...
	\For{$s \in\{1,\dots,S\}$}{
		...
		\For{$p \in \{1,\dots,P\}$}{
			...
			$z_{p} \gets Gumbel(0,1)$\;
		\HiLi	$\ket{\theta_p} \gets \ttheta_p$; \hspace{5em} \Comment{      Load proposal onto quantum computer.}      
		}
	\HiLi	$\hat{p} \gets \argmin_{p=0,\dots,P}\Big(f(p):= -\big(z_{p}+ \log\pi(\ttheta_{p}) \big)\Big)$\;  \HiLi \hspace{10em}\Comment{\hspace{0.5em} $\order{\sqrt{P}}$ Alg \ref{alg:min} with warm-start at $p=0$.}
		$\ttheta^{(s)} \gets \ttheta_{\hat{p}}$\;
	}
	\Return{$\ttheta^{(1)}, \dots, \ttheta^{(S)}$}\ .
